...
if [[ ! $(cat $TMPFILE) == *"No outputs found"* ]]; then
  source "$TMPFILE"
  kubectl delete -n argocd applicationset Bingo-UAT-workloads
  echo "Deleting ingress/svc for game-2048, takes a few minutes for Load Balancer to be deleted"
  #kubectl delete -n bingo ing bingo-java-ingress 
  #kubectl delete -n bingo ingbingo-rabbitmq-ingress
